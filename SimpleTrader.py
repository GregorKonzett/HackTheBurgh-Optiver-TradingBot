...
while True:
	...
	if("TYPE=TRADE|FEEDCODE=ESX-FUTURE|SIDE=BID" in data):
		...
		if (pred_bid_ESX > 0 and pred_bid_ESX + THRESHHOLD < bidPrice):
			#Sell
		...
		if (pred_ask_ESX > 0 and pred_ask_ESX - THRESHHOLD >= askPrice):
			#Buy
		...
		if (pred_bid_SP > 0 and pred_bid_SP + THRESHHOLD < bidPrice):
		...
		if (pred_ask_SP > 0 and pred_ask_SP - THRESHHOLD > askPrice):
			#Buy
